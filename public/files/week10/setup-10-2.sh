...
create_subnets() {
    ...
    # Public Subnet 1 확인/생성 (${REGION}a)
    ...
    # Public Subnet 2 확인/생성 (${REGION}b)
    ...
    # Private Subnet 1 확인/생성 (${REGION}a)
    ...
    # Private Subnet 2 확인/생성 (${REGION}b)
...
show_completion_summary() {
    ...
    echo "  ✅ Private Route Table: CloudArchitect-Lab-Private-RT ($PRIVATE_RT_ID)"
    ...
    echo "  2. ECS 클러스터 생성:"
    echo "     • 클러스터 이름: CloudArchitect-Lab-Cluster"
    ...
    echo "  4. IAM 역할 설정:"
    echo "     • Task Execution Role: Create new role 선택"
    echo "     • ⚠️ AWS가 자동으로 ecsTaskExecutionRole을 생성합니다"
    echo "  5. ECS 서비스 생성 및 배포:"
    echo "     • 서비스 이름: cloudarchitect-lab-service"
    echo "     • 원하는 작업 수: 2"
    echo "     • 네트워킹: Private 서브넷 선택"
    echo "     • 로드 밸런서: Application Load Balancer 연결"
