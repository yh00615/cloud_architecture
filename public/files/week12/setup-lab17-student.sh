...
# ================================
# Lab17: AWS Backup 서비스 - 자동화된 백업 및 복원
# ================================
...
show_creation_plan() {
    ...
    if [ "$existing_ec2" != "None" ] && [ -n "$existing_ec2" ]; then
        ...
        echo "  • 백업용 태그: Project=CloudArchitect, Lab=Lab17"
...
confirm_creation() {
    echo ""
    read -p "위 계획대로 Lab17 리소스를 생성하시겠습니까? (y/N): " confirm
    if [[ ! $confirm =~ ^[Yy]$ ]]; then
        show_info "Lab17 설정이 취소되었습니다."
        exit 0
    fi
    show_info "Lab17 리소스 생성을 시작합니다..."
    echo ""
}
...
show_completion_summary() {
    echo ""
    show_success "🎉 Lab17 사전 구축이 완료되었습니다!"
    echo ""
...
create_vpc() {
    ...
    VPC_ID=$(aws ec2 create-vpc \
        ...
        --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=CloudArchitect-Lab-VPC},{Key=Project,Value=CloudArchitect},{Key=Environment,Value=Lab},{Key=Component,Value=Network},{Key=CreatedBy,Value=setup-lab17-student.sh}]' \
...
create_internet_gateway() {
    ...
    IGW_ID=$(aws ec2 create-internet-gateway \
        --tag-specifications 'ResourceType=internet-gateway,Tags=[{Key=Name,Value=CloudArchitect-Lab-IGW},{Key=Project,Value=CloudArchitect},{Key=Environment,Value=Lab},{Key=Component,Value=Network},{Key=CreatedBy,Value=setup-lab17-student.sh}]' \
...
create_public_subnet() {
    ...
    PUBLIC_SUBNET_ID=$(aws ec2 create-subnet \
        ...
        --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=CloudArchitect-Lab-Public-Subnet},{Key=Project,Value=CloudArchitect},{Key=Environment,Value=Lab},{Key=Component,Value=Network},{Key=Type,Value=Public},{Key=CreatedBy,Value=setup-lab17-student.sh}]' \
...
create_route_table() {
    ...
    PUBLIC_RT_ID=$(aws ec2 create-route-table \
        --vpc-id $VPC_ID \
        --tag-specifications 'ResourceType=route-table,Tags=[{Key=Name,Value=CloudArchitect-Lab-Public-RT},{Key=Project,Value=CloudArchitect},{Key=Environment,Value=Lab},{Key=Component,Value=Network},{Key=Type,Value=Public},{Key=CreatedBy,Value=setup-lab17-student.sh}]' \
...
create_ec2_security_group() {
    ...
    EC2_SG_ID=$(aws ec2 create-security-group \
        --group-name $sg_name \
        --description "CloudArchitect Lab17 EC2 Security Group for Backup Testing" \
        --vpc-id $VPC_ID \
        --tag-specifications 'ResourceType=security-group,Tags=[{Key=Name,Value=CloudArchitect-Lab-EC2-SG},{Key=Project,Value=CloudArchitect},{Key=Environment,Value=Lab},{Key=Component,Value=Security},{Key=CreatedBy,Value=setup-lab17-student.sh}]' \
...
create_backup_role() {
    ...
    BACKUP_ROLE_ARN=$(aws iam create-role \
        ...
        --description "CloudArchitect Lab17 AWS Backup Service Role" \
        --tags Key=Name,Value=CloudArchitect-Lab-BackupRole Key=Project,Value=CloudArchitect Key=Environment,Value=Lab Key=Component,Value=IAM Key=CreatedBy,Value=setup-lab17-student.sh \
...
<html>
<head>
    <title>CloudArchitect Lab17 - Backup Test Instance</title>
    <style>
...
<body>
    <div class="header">
        ...
        <p>CloudArchitect Lab17 - AWS Backup Service</p>
    </div>
    <div class="content">
        <div class="info">
            <h3>인스턴스 정보</h3>
            ...
            <p><strong>백업 태그:</strong> Project=CloudArchitect, Lab=Lab17</p>
        </div>
        <h3>백업 실습 내용</h3>
        <ul>
...
CloudArchitect Lab17 - AWS Backup 실습 데이터
생성 시간: $(date)
...
'
    ...
    EC2_INSTANCE_ID=$(aws ec2 run-instances \
        ...
        --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=CloudArchitect-Lab-TestInstance},{Key=Project,Value=CloudArchitect},{Key=Lab,Value=Lab17},{Key=Environment,Value=Lab},{Key=Component,Value=Compute},{Key=Purpose,Value=Backup-Test},{Key=CreatedBy,Value=setup-lab17-student.sh}]' \
...
main() {
    ...
    # 헤더 표시
    echo "================================"
    echo "Lab17: AWS Backup 서비스 - 자동화된 백업 및 복원"
    echo "================================"
