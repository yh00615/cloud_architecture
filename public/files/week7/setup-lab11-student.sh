...
setup_network_resources() {
    ...
    echo "VPC_ID=$vpc_id" > /tmp/lab11-resources.env
    echo "IGW_ID=$igw_id" >> /tmp/lab11-resources.env
}

# 서브넷 및 보안그룹 생성 함수
create_subnet_and_security_group() {
    source /tmp/lab11-resources.env
    ...
    subnet_id=$(aws ec2 describe-subnets \
        --filters \
            ...
            "Name=availability-zone,Values=ap-northeast-2a" \
            "Name=state,Values=available" \
        --query 'Subnets[0].SubnetId' \
        --output text 2>/dev/null)
    
    if [ "$subnet_id" != "None" ] && [ -n "$subnet_id" ]; then
        show_success "기존 Public Subnet 재사용: CloudArchitect-Lab-Public-Subnet ($subnet_id)"
    else
        ...
        show_success "Public Subnet 생성 완료: CloudArchitect-Lab-Public-Subnet ($subnet_id)"
    fi
    ...
    if [ "$route_table_id" != "None" ] && [ -n "$route_table_id" ]; then
        show_success "기존 Route Table 재사용: $route_table_id"
    else
        ...
        show_success "Route Table 생성 및 연결 완료: $route_table_id"
    fi
    ...
    echo "SUBNET_ID=$subnet_id" >> /tmp/lab11-resources.env
    echo "SG_ID=$sg_id" >> /tmp/lab11-resources.env
}
...
Type=oneshot
User=ec2-user
ExecStart=/bin/bash -c 'sleep 60 && /home/ec2-user/generate-traffic.sh'
RemainAfterExit=no
...
# 초기 트래픽 즉시 생성 (백그라운드)
(sleep 60 && /home/ec2-user/generate-traffic.sh) &
...
EOF
    ...
    echo "INSTANCE_ID=$instance_id" >> /tmp/lab11-resources.env
    echo "PUBLIC_IP=$public_ip" >> /tmp/lab11-resources.env
    
    show_success "인스턴스 실행 중: $instance_id (IP: $public_ip)"
}
...
verify_cloudwatch_agent() {
    ...
    show_info "CloudWatch Agent 초기화 대기 중... (약 2분)"
    sleep 120
    
    # 로그 그룹 확인
    show_info "CloudWatch Logs 그룹 확인 중..."
    
    access_log_group=$(aws logs describe-log-groups \
        --log-group-name-prefix "/aws/ec2/nginx/access" \
        --query 'logGroups[0].logGroupName' \
        --output text 2>/dev/null || echo "None")
    
    error_log_group=$(aws logs describe-log-groups \
    ...
    if [ "$access_log_group" != "None" ] && [ -n "$access_log_group" ]; then
        ...
        show_warning "Access 로그 그룹이 아직 생성되지 않았습니다. 몇 분 후 다시 확인해주세요."
    fi
    
    if [ "$error_log_group" != "None" ] && [ -n "$error_log_group" ]; then
        show_success "Error 로그 그룹 생성됨: $error_log_group"
    else
        show_warning "Error 로그 그룹이 아직 생성되지 않았습니다. 몇 분 후 다시 확인해주세요."
    fi
}
...
show_completion_summary() {
    ...
    echo "  ✅ Public Subnet: CloudArchitect-Lab-Public-Subnet ($SUBNET_ID)"
    echo "  ✅ Security Group: CloudArchitect-Lab-Web-SG ($SG_ID)"
    ...
    show_success "✅ Lab11 사전 환경 구축 완료"
}
