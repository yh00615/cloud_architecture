...
34. **Items returned**와 **Items scanned** 수치를 확인합니다.
...
35. **Scan or query items** 섹션에서 드롭다운을 **Query**로 변경합니다.

36. **Partition key** 필드에 `user001`을 입력합니다.

37. [[Run]] 버튼을 클릭합니다.

38. `user001` (김철수) 사용자의 정보만 정확히 조회되는지 확인합니다.
...
39. **Items returned**이 `1`이고 **Items scanned**도 `1`인 것을 확인합니다.
...
40. **Scan or query items** 섹션에서 드롭다운을 다시 **Scan**으로 변경하고 [[Run]] 버튼을 클릭하여 전체 아이템을 조회합니다.

41. 하단 결과 목록에서 `user002` 아이템의 체크박스를 선택합니다.

42. 화면 오른쪽 상단의 **Actions** > **Edit item**을 선택합니다.

> [!TIP]
> 또는 각 값 위에 마우스를 올리면 나타나는 연필 모양 아이콘(✏️)을 클릭하여 해당 필드를 바로 수정할 수도 있습니다. 예를 들어 `age` 값 위에 마우스를 올리면 연필 아이콘이 나타나고, 클릭하면 바로 편집 창이 열립니다.

43. **age** 값을 `24`로 변경합니다.

44. [[Save]] 버튼을 클릭합니다.

45. 변경된 내용이 반영되었는지 확인합니다.
