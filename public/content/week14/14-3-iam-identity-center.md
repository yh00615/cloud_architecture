...
17. Builder ID 생성은 VS Code에서 Amazon Q 확장을 설치할 때 함께 진행됩니다. 별도로 미리 생성할 필요가 없습니다.
...
18. VS Code가 설치되어 있지 않은 경우 VS Code 공식 사이트에서 다운로드하고 설치합니다.

19. VS Code를 실행하고 왼쪽 사이드바의 **Extensions** 아이콘(사각형 4개 모양)을 선택합니다.

20. Extensions 검색창에 `Amazon Q`를 입력합니다.

21. 검색 결과에서 **Amazon Q** 확장 (게시자: Amazon Web Services)을 찾아 [[Install]] 버튼을 클릭합니다.

> [!IMPORTANT]
> 여러 Amazon Q 관련 확장이 표시될 수 있습니다. 반드시 게시자가 "Amazon Web Services"인 공식 확장을 선택합니다. 게시자 이름은 확장 이름 아래에 표시됩니다.

22. 설치가 완료되면 VS Code를 재시작합니다.

23. 왼쪽 사이드바에서 Amazon Q 아이콘이 표시되는지 확인합니다.
...
24. VS Code에서 왼쪽 사이드바의 **Amazon Q 아이콘**을 선택합니다.

25. Amazon Q 패널에서 [[Start]] 버튼을 클릭합니다.

26. 인증 방법 선택 화면에서 **Personal account - Free to start with a Builder ID**를 선택합니다.

27. 브라우저가 자동으로 열리면서 Builder ID 로그인 페이지가 표시됩니다.

28. Builder ID가 이미 있는 경우 이메일 주소를 입력하고 로그인합니다.

29. Builder ID가 없는 경우 [[Create AWS Builder ID]] 버튼을 클릭하여 다음 정보를 입력합니다:
...
30. 브라우저에서 권한 승인 단계가 나타나면 [[Open]] 버튼을 클릭하여 VS Code 연동을 허용합니다.

31. 로그인 후 브라우저 창에서 액세스 허용을 선택합니다.

32. 인증이 완료되면 VS Code에서 Amazon Q 패널의 "Connected" 상태를 확인합니다.
...
33. VS Code에서 새 파일을 생성하고 `hello.py`로 저장합니다.
...
35. **Enter**를 눌러 새 줄로 이동하고 약 3-5초간 기다립니다. Amazon Q가 자동으로 코드를 제안합니다.

36. 제안된 코드가 회색으로 표시되면 **Tab** 키를 눌러 수락합니다. 추가 제안이 나타나면 계속 **Tab** 키로 수락합니다.
...
37. VS Code 왼쪽 사이드바의 **Amazon Q 아이콘**을 선택하여 채팅 패널을 엽니다.

38. 생성된 피보나치 함수 코드를 전체 선택하여 복사합니다.

39. Amazon Q 채팅창에 `이 코드를 설명해주세요`라고 입력하고 **Enter**를 누릅니다.

40. AI의 설명을 확인한 후, `이 코드를 더 효율적으로 만들어주세요`라고 요청합니다.

41. 다른 언어로도 테스트합니다: `이 함수를 JavaScript로 변환해주세요`
...
42. 새 파일을 생성하고 `api_test.py`로 저장합니다.

43. Amazon Q 채팅창에 다음과 같이 요청합니다: `REST API를 호출하는 Python 코드를 작성해주세요. requests 라이브러리를 사용하고, GET과 POST 메서드 예시를 포함해주세요.`

44. 생성된 코드를 복사하여 파일에 붙여넣고 저장합니다.

45. 추가로 `에러 처리와 타임아웃 설정도 추가해주세요`라고 요청하여 더 완성도 높은 코드를 받아봅니다.

### 7.2 테스트 코드 생성

46. 앞서 생성한 피보나치 함수 코드를 복사합니다.

47. Amazon Q에게 `다음 함수에 대한 pytest 단위 테스트 코드를 작성해주세요`라고 요청하고 함수 코드를 붙여넣습니다.

48. 새 파일 `test_fibonacci.py`를 생성하고 생성된 테스트 코드를 저장합니다.

### 7.3 코드 리뷰 및 최적화

49. 작성한 코드 중 하나를 선택하여 전체 코드를 복사합니다.

50. Amazon Q에게 `다음 코드를 리뷰해주세요. 개선점과 모범 사례를 제안해주세요`라고 요청합니다.

51. AI의 제안을 확인한 후, `이 코드에 주석을 추가하고 docstring을 작성해주세요`라고 요청하여 문서화도 개선합니다.
...
52. VS Code에서 Amazon Q 채팅 패널을 엽니다.

53. 채팅 패널 하단에 `</>` 아이콘이 활성화되어 있는지 확인합니다. 비활성화되어 있다면 클릭하여 활성화합니다.
...
54. 새 폴더를 만들고 VS Code에서 엽니다.

55. Amazon Q 채팅창에 다음과 같이 요청합니다:
...
56. Amazon Q가 작업을 시작하면 진행 상황을 확인합니다. Amazon Q는 자동으로 파일을 생성하고 코드를 작성합니다.

57. 생성된 파일들을 열어 코드를 확인합니다.
...
59. Amazon Q가 파일을 자동으로 수정하는 것을 확인합니다.

60. VS Code의 소스 제어 탭에서 변경 사항을 확인합니다. diff 뷰에서 어떤 부분이 수정되었는지 확인할 수 있습니다.
...
61. `index.html` 파일을 브라우저에서 열어 게임을 실행합니다.
