...
16. **Distribution type**에서 `Single website or app`을 선택합니다.

17. [[Next]] 버튼을 클릭합니다.

18. **Origin type**에서 `Amazon S3`를 선택합니다.

19. **S3 origin**에서 [[Browse S3]] 버튼을 클릭합니다.

20. 버킷 목록에서 `cloudarchitect-lab-s3website-[계정ID]` 버킷을 선택하고 [[Choose]] 버튼을 클릭합니다.

21. **Allow private S3 bucket access to CloudFront - Recommended**가 선택되어 있는지 확인합니다.

22. **Origin settings**에서 `Use recommended origin settings`를 선택합니다.

23. **Cache settings**에서 `Use recommended cache settings tailored to serving S3 content`를 선택합니다.

24. [[Next]] 버튼을 클릭합니다.

25. **Do not enable security protections**를 선택합니다.

26. [[Next]] 버튼을 클릭합니다.

27. 설정을 검토한 후 [[Create distribution]] 버튼을 클릭합니다.
...
28. 배포 생성 후 상단에 나타나는 배너에서 [[Copy policy]] 버튼을 클릭하여 S3 버킷 정책을 복사합니다.

> [!TIP]
> 배너가 사라진 경우, 배포 상세 페이지의 **Origins** 탭에서 오리진을 선택하고 [[Edit]] 버튼을 클릭하면 정책을 다시 확인할 수 있습니다.

29. 상단 검색창에서 `S3`를 검색하고 **S3**를 선택합니다.

30. `cloudarchitect-lab-s3website-[계정ID]` 버킷을 선택합니다.

31. 상단 탭 중 **Permissions** 탭을 선택합니다.

32. **Bucket policy** 섹션에서 [[Edit]] 버튼을 클릭합니다.

33. 기존 정책을 모두 삭제하고 복사한 CloudFront OAC 정책을 붙여넣습니다.
...
35. Amazon CloudFront 콘솔로 이동하여 배포의 **Last modified** 필드가 날짜/시간으로 변경되었는지 확인합니다.

36. 배포 상세 페이지의 **General** 탭에서 **Distribution domain name**을 복사합니다 (예: `d111111abcdef8.cloudfront.net`).

37. 새 브라우저 탭을 열고 `https://[복사한 Domain name]/index.html`로 접속합니다.

38. 웹사이트가 정상적으로 로드되는지 확인합니다.

39. 브라우저 **개발자 도구**(F12)를 열고 **Network** 탭을 선택합니다.

40. 페이지를 새로고침하고 `index.html` 요청을 선택하여 응답 헤더를 확인합니다.

41. `X-Cache` 헤더 값을 확인합니다:
- 첫 번째 요청: `Miss from cloudfront` (엣지 로케이션에 캐시가 없어 오리진에서 가져옴)
- 이후 요청: `Hit from cloudfront` (엣지 로케이션의 캐시에서 응답)

42. 페이지를 여러 번 새로고침하여 `X-Cache` 값이 `Hit from cloudfront`로 변경되는지 확인합니다.
...
43. Amazon S3 콘솔에서 `cloudarchitect-lab-s3website-[계정ID]` 버킷을 선택합니다.

44. `index.html` 파일의 체크박스를 선택하고 [[Download]] 버튼을 클릭하여 로컬에 저장합니다.

45. 다운로드한 파일을 텍스트 편집기로 열고 내용을 수정합니다 (예: "Hello world!" → "Hello CloudFront!").

46. 수정한 파일을 S3 버킷에 다시 업로드합니다 ([[Upload]] 버튼 선택).

47. CloudFront 도메인으로 접근하여 변경사항이 즉시 반영되지 않는 것을 확인합니다 (캐시된 이전 콘텐츠가 표시됨).

> [!NOTE]
> CloudFront는 기본적으로 24시간 동안 콘텐츠를 캐시합니다. 즉시 업데이트를 반영하려면 캐시 무효화(Invalidation)를 수행해야 합니다.

48. Amazon CloudFront 콘솔에서 생성한 배포를 선택합니다.

49. 상단 탭 중 **Invalidations** 탭을 선택합니다.

50. [[Create invalidation]] 버튼을 클릭합니다.

51. **Object paths**에 `/index.html`을 입력합니다.

52. [[Create invalidation]] 버튼을 클릭합니다.

53. 무효화 상태가 "Completed"가 될 때까지 기다립니다 (약 1-2분 소요).

54. 웹사이트를 새로고침하여 변경사항이 반영되었는지 확인합니다 ("Hello CloudFront!" 표시).
