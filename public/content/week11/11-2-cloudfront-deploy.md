...
> **포함 파일:**
> 
> **setup-11-2.sh** - 사전 환경 구축 스크립트
> - **목적**: CloudFront 실습을 위한 S3 버킷과 샘플 웹 파일 자동 구축
> - **생성 리소스**:
>   - S3 버킷 (cloudarchitect-lab-s3website-[계정ID])
>   - 샘플 웹 파일 (index.html, error.html)
>   - 프라이빗 버킷 설정 (CloudFront OAC를 통해서만 접근 가능)
> - **실행 시간**: 약 1-2분
> - **활용**: 태스크 2-5에서 CloudFront 배포를 생성하고, OAC로 S3와 안전하게 연결하며, 캐시 무효화를 테스트합니다
>
> **cleanup-11-2.sh** - 리소스 정리 스크립트
> - **목적**: 실습에서 생성한 모든 리소스를 안전한 순서로 자동 삭제
> - **삭제 리소스**: CloudFront 배포, S3 버킷 및 객체
> - **실행 시간**: 약 10-15분 (CloudFront 배포 삭제 시간 포함)
>
> **사용 태스크:**
> - 태스크 0: 사전 환경 구축 (setup-11-2.sh 실행)
> - 리소스 정리: 실습 완료 후 cleanup-11-2.sh 실행
...
이 실습에서는 **Amazon CloudFront**를 사용하여 전 세계 사용자에게 빠르게 콘텐츠를 제공하는 CDN을 구축하는 방법을 학습합니다. 이를 위해 다음과 같은 환경이 필요합니다:

**구축되는 인프라:**
- **S3 버킷**: 정적 웹사이트 파일(HTML, CSS, 이미지)을 저장하는 오리진입니다
- **샘플 웹 파일**: index.html과 error.html이 업로드되어 즉시 테스트할 수 있습니다
- **프라이빗 버킷 설정**: 퍼블릭 액세스를 차단하여 CloudFront를 통해서만 접근 가능하도록 합니다

**실습에서의 활용:**
- **태스크 1**: 생성된 S3 버킷과 샘플 파일을 확인합니다
- **태스크 2**: CloudFront 배포를 생성하고 OAC로 S3와 안전하게 연결합니다
- **태스크 3**: CloudFront 도메인으로 웹사이트에 접속하여 CDN을 테스트합니다
- **태스크 4**: 파일을 수정하고 캐시 무효화를 통해 즉시 반영합니다
- **태스크 5**: CloudFront 통계와 로그를 확인하여 성능을 분석합니다

> [!TIP]
> 사전 환경 구축 스크립트는 S3 버킷과 샘플 파일을 자동으로 준비하므로, 여러분은 CloudFront의 핵심 CDN 기능 학습에만 집중할 수 있습니다.
...
|------------|------------|----------------|
| S3 버킷 | cloudarchitect-lab-s3website-[계정 ID] | CloudFront의 오리진 (콘텐츠 저장소) |
| index.html | 메인 페이지 | CloudFront를 통해 제공되는 홈페이지 |
| error.html | 에러 페이지 | 404 에러 발생 시 표시되는 페이지 |

8. 출력 메시지에서 **S3 버킷 이름**을 확인하고 메모합니다 (태스크 2에서 사용).
