...
3. **Bucket name**에 `cloudarchitect-demo-s3website-[계정ID]`를 입력합니다.
...
<body>
    <div class="container">
        ...
        <footer>
            <p>&copy; 2024 CloudArchitect 실습 - S3 정적 웹사이트 호스팅</p>
        </footer>
    </div>
    <script>
...
10. S3 콘솔에서 생성한 버킷을 선택합니다.

11. **Objects** 탭에서 [[Upload]] 버튼을 클릭합니다.

12. [[Add files]] 버튼을 클릭하여 `index.html`과 `style.css` 파일을 선택합니다.

13. 파일이 추가되었는지 확인하고 [[Upload]] 버튼을 클릭합니다.

14. 업로드가 완료되면 [[Close]] 버튼을 클릭합니다.
...
15. 버킷 상세 페이지 상단의 탭 메뉴에서 **Properties** 탭을 선택합니다. (Objects, Properties, Permissions, Metrics, Management, Access Points 순서로 나열되어 있습니다.)

16. **Properties** 탭 페이지에서 아래로 스크롤하여 맨 하단의 **Static website hosting** 섹션을 찾습니다.

17. [[Edit]] 버튼을 클릭합니다.

18. **Static website hosting**에서 `Enable`을 선택합니다.

19. **Hosting type**에서 `Host a static website`를 선택합니다.

20. **Index document** 필드에 `index.html`을 입력합니다.

21. **Error document** 필드에 `error.html`을 입력합니다.

22. [[Save changes]] 버튼을 클릭합니다.

### 3.2 웹사이트 엔드포인트 URL 확인

23. **Properties** 탭의 **Static website hosting** 섹션에서 **Bucket website endpoint** URL을 확인합니다.

24. URL을 복사하여 메모장에 저장합니다.

> [!OUTPUT]
> ```
> http://cloudarchitect-demo-s3website-123456789012.s3-website.ap-northeast-2.amazonaws.com
> ```

> [!IMPORTANT]
> 이 URL은 나중에 웹사이트 접속 테스트에 사용합니다. 반드시 메모장에 저장합니다.
...
25. 버킷 상세 페이지 상단의 탭 메뉴에서 **Permissions** 탭을 선택합니다.

26. **Bucket policy** 섹션에서 [[Edit]] 버튼을 클릭합니다.

27. 다음 정책을 입력합니다 (`YOUR-BUCKET-NAME`을 본인의 버킷 이름으로 변경):
...
28. `YOUR-BUCKET-NAME` 부분을 본인의 실제 버킷 이름(예: `cloudarchitect-demo-s3website-123456789012`)으로 변경합니다.
...
30. 새 브라우저 탭을 열고 메모장에 저장한 웹사이트 엔드포인트 URL을 붙여넣고 Enter를 누릅니다.

31. **CloudArchitect S3 웹사이트 실습** 페이지가 표시되는지 확인합니다.

32. CSS 스타일이 올바르게 적용되었는지 확인합니다 (보라색 그라데이션 배경, 카드 레이아웃).

33. JavaScript가 작동하여 현재 날짜가 표시되는지 확인합니다.
...
35. S3 버킷에 `error.html` 파일을 업로드합니다.

36. 웹사이트 URL 뒤에 `/nonexistent.html`을 추가하여 접속합니다.

37. 사용자 정의 404 오류 페이지가 표시되는지 확인합니다.
