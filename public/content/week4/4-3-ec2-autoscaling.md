...
```bash
chmod +x setup-4-3.sh cleanup-4-3.sh
./setup-4-3.sh
```
...
9. AWS Management Console에 로그인한 후 상단 검색창에 `EC2`를 검색하고 **EC2**를 선택합니다.

10. 왼쪽 메뉴에서 **Instances**를 선택합니다.

11. **CloudArchitect-Lab-WebServer** 인스턴스를 찾아 선택합니다.

12. 인스턴스 상태가 "Running"인지 확인합니다.
...
15. AWS Management Console에 로그인한 후 상단 검색창에 `VPC`를 검색하고 **VPC**를 선택합니다.

16. 왼쪽 메뉴에서 **Subnets**를 선택합니다.

17. 다음 2개의 서브넷이 있는지 확인합니다:

- **CloudArchitect-Lab-Public-Subnet-1** (ap-northeast-2a)
- **CloudArchitect-Lab-Public-Subnet-2** (ap-northeast-2b)
...
20. **Inbound rules** 탭에서 SSH(22)와 HTTP(80) 규칙이 설정되어 있는지 확인합니다.
...
21. AWS Management Console에 로그인한 후 상단 검색창에 `EC2`를 검색하고 **EC2**를 선택합니다.

22. EC2 콘솔의 왼쪽 메뉴에서 **Instances** 섹션 아래의 **Launch Templates**를 선택합니다.

23. [[Create launch template]] 버튼을 클릭합니다.

24. **Launch template name**에 `CloudArchitect-Lab-WebServer-Template`를 입력합니다.

25. **Template version description**에 `Web server template for Auto Scaling`를 입력합니다.

26. **Auto Scaling guidance** 체크박스를 체크합니다.
...
27. **Application and OS Images** 섹션에서 **Quick Start** 탭이 선택되어 있는지 확인합니다.

28. **Quick Start** 탭에서 **Amazon Linux**를 선택합니다.

29. **Amazon Machine Image (AMI)** 드롭다운에서 `Amazon Linux 2023 AMI`를 선택합니다.

30. **Instance type**에서 `t3.micro`를 선택합니다.

31. **Key pair (login)**에서 `Don't include in launch template`를 선택합니다.

### 2.3 보안 설정

32. **Network settings** 섹션에서 **Firewall (security groups)**의 `Select existing security group`를 선택합니다.

33. **Common security groups**에서 **CloudArchitect-Lab-WebServer-SG**를 선택합니다.

### 2.4 User Data 스크립트 설정

34. **Advanced details** 섹션을 확장합니다. **Advanced details**는 **Network settings** 아래에 접힌 상태로 있습니다. 섹션 제목을 클릭하여 확장합니다.

35. **Advanced details** 섹션 내에서 맨 아래로 스크롤하면 **User data** 텍스트 박스가 있습니다. 다음 스크립트를 입력합니다:
...
36. [[Create launch template]] 버튼을 클릭합니다.
...
37. EC2 콘솔에서 왼쪽 메뉴의 맨 아래 **Auto Scaling** 섹션 아래의 **Auto Scaling Groups**를 선택합니다.

38. [[Create Auto Scaling group]] 버튼을 클릭합니다.

39. **Auto Scaling group name**에 `CloudArchitect-Lab-ASG`를 입력합니다.

40. **Launch template**에서 **CloudArchitect-Lab-WebServer-Template**를 선택합니다.

41. [[Next]] 버튼을 클릭합니다.

### 3.2 네트워크 설정

42. **VPC**에서 **CloudArchitect-Lab-VPC**를 선택합니다.

43. **Availability Zones and subnets**에서 2개의 Public 서브넷을 모두 선택합니다.

44. [[Next]] 버튼을 클릭합니다.

### 3.3 로드 밸런싱 설정

45. **Load balancing**에서 **Attach to a new load balancer**를 선택합니다.

46. **Load balancer type**에서 **Application Load Balancer**를 선택합니다.

47. **Load balancer name**에 `CloudArchitect-Lab-ALB`를 입력합니다.

48. **Load balancer scheme**에서 **Internet-facing**를 선택합니다.

49. **Listeners and routing**에서 **Create a target group**를 선택합니다.

50. **New target group name**에 `CloudArchitect-Lab-TG`를 입력합니다.
...
53. **Scaling policies**에서 **Target tracking scaling policy**를 선택합니다.

54. **Metric type**에서 **Average CPU utilization**를 선택합니다.

55. **Target value**에 `70`를 입력합니다.

56. [[Next]] 버튼을 클릭합니다.

### 3.5 알림 설정

57. **Add notifications** 단계에서 [[Next]] 버튼을 클릭합니다.

### 3.6 태그 설정

58. **Add tags** 단계에서 [[Add tag]] 버튼을 클릭합니다.

59. 첫 번째 태그를 다음과 같이 입력합니다:

- **Key**: `Name`
- **Value**: `CloudArchitect-Lab-ASG`
- **Tag new instances**: 체크 ✅

60. [[Add tag]] 버튼을 다시 클릭하고 두 번째 태그를 추가합니다:
...
63. [[Create Auto Scaling group]] 버튼을 클릭합니다.
...
64. EC2 콘솔에서 왼쪽 메뉴의 **Auto Scaling Groups**를 선택합니다.

65. **CloudArchitect-Lab-ASG**를 선택합니다.

66. **Instance management** 탭을 선택합니다.

67. 인스턴스 2개가 "InService" 상태인지 확인합니다.
...
70. **State**가 "Active"인지 확인합니다.

71. **DNS name**을 복사합니다 (예: **CloudArchitect-Lab-ALB-123456789.ap-northeast-2.elb.amazonaws.com**).

72. 새 브라우저 탭을 열고 `http://[복사한 DNS name]`으로 접속합니다.

73. "Lab06: Auto Scaling Web Server" 페이지가 표시되는지 확인합니다.

74. 페이지를 여러 번 새로고침(F5)하여 **인스턴스 ID**와 **가용 영역**이 변경되는지 확인합니다.
...
78. 등록된 인스턴스들의 **Health status**가 "healthy"인지 확인합니다.
...
79. EC2 콘솔에서 왼쪽 메뉴의 **Instances**를 선택합니다.

80. Auto Scaling Group에서 생성된 인스턴스 중 하나를 선택합니다 (이름에 CloudArchitect-Lab-ASG가 포함된 인스턴스).

81. **Instance state** > **Terminate instance**를 선택합니다.

82. 확인 창에서 [[Terminate]] 버튼을 클릭합니다.

83. 왼쪽 메뉴에서 **Auto Scaling Groups**를 선택합니다.

84. **CloudArchitect-Lab-ASG**를 선택한 후 **Activity** 탭을 확인합니다.

85. 종료된 인스턴스를 대체할 새 인스턴스가 자동으로 시작되는 Activity 로그를 확인합니다.
...
86. EC2 콘솔에서 왼쪽 메뉴의 **Auto Scaling Groups**를 선택합니다.

87. **CloudArchitect-Lab-ASG**를 선택합니다.

88. **Actions** > **Delete**를 선택합니다.

89. 확인 텍스트를 입력하고 삭제합니다.
...
92. **Actions** > **Delete load balancer**를 선택합니다.
...
96. **Actions** > **Delete**를 선택합니다.

### 5.3 Launch Template 삭제

97. 왼쪽 메뉴에서 **Launch Templates**를 선택합니다.

98. **CloudArchitect-Lab-WebServer-Template**를 선택합니다.

99. **Actions** > **Delete template**를 선택합니다.

100. 확인 텍스트를 입력하고 삭제합니다.

### 5.4 Cleanup 스크립트 실행

101. CloudShell에서 cleanup 스크립트를 실행하여 사전 구축 리소스를 정리합니다:
