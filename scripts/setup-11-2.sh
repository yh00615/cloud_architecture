...
# ===========================================
# Lab16: Amazon CloudFront 배포 구성 - 글로벌 콘텐츠 전송 네트워크
# 목적: Lab16 CloudFront 실습을 위한 S3 버킷 및 웹사이트 환경 생성
# 예상 시간: 약 15분
...
show_creation_plan() {
    ...
    echo "    📝 환경 파일: lab16-prerequisites.env (cleanup용)"
    echo ""
    echo "  🎯 핵심 학습 목표:"
...
confirm_creation() {
    echo ""
    read -p "위 계획대로 Lab16 리소스를 생성하시겠습니까? (y/N): " confirm
    if [[ ! $confirm =~ ^[Yy]$ ]]; then
        show_info "Lab16 설정이 취소되었습니다."
        exit 0
    fi
    show_info "Lab16 리소스 생성을 시작합니다..."
    echo ""
}
    ...
    if aws s3 mb "s3://$bucket_name" --region "$region" >/dev/null 2>&1; then
        ...
        aws s3api put-bucket-tagging \
            ...
            --tagging "TagSet=[
                ...
                {Key=Demo,Value=Lab16},
                {Key=Component,Value=Storage},
                {Key=Environment,Value=Lab},
                {Key=CreatedBy,Value=setup-lab16-student.sh}
            ]" 2>/dev/null || true
...
show_completion_summary() {
    ...
    echo ""
    show_success "🎉 Lab16 CloudFront 인프라 구축이 완료되었습니다!"
    echo ""
    ...
    echo "  ✅ 환경 파일: lab16-prerequisites.env"
    echo ""
    ...
    show_success "Lab16 스크립트 실행 완료"
    
    # 환경 변수 저장 (cleanup에서 사용)
    cat > lab16-prerequisites.env << EOF
...
main() {
    ...
    # 헤더 표시
    echo "================================"
    echo "Lab16: Amazon CloudFront 배포 구성 - 글로벌 콘텐츠 전송 네트워크"
    echo "================================"
