...
# ================================
# Lab06: Amazon EC2 Auto Scaling 구성 정리
# ================================
# 목적: Lab06에서 생성된 모든 AWS 리소스를 안전하게 정리
# ================================
...
show_deletion_plan() {
    ...
    local igw_id=""
    local public_subnet1_id=""
    local public_subnet2_id=""
    local sg_id=""
    ...
    if [ "$vpc_id" != "None" ] && [ -n "$vpc_id" ]; then
        ...
        # Public Subnets 확인
        public_subnet1_id=$(aws ec2 describe-subnets \
            --filters "Name=tag:Name,Values=CloudArchitect-Lab-Public-Subnet-1" "Name=vpc-id,Values=$vpc_id" \
            --query 'Subnets[0].SubnetId' \
            --output text 2>/dev/null)
        
        public_subnet2_id=$(aws ec2 describe-subnets \
            --filters "Name=tag:Name,Values=CloudArchitect-Lab-Public-Subnet-2" "Name=vpc-id,Values=$vpc_id" \
            --query 'Subnets[0].SubnetId' \
            --output text 2>/dev/null)
        
        if [ "$public_subnet1_id" != "None" ] && [ -n "$public_subnet1_id" ]; then
            echo "✅ Public Subnet 1: CloudArchitect-Lab-Public-Subnet-1 ($public_subnet1_id)"
        fi
        ...
        sg_id=$(aws ec2 describe-security-groups \
            --filters "Name=tag:Name,Values=CloudArchitect-Lab-WebServer-SG" "Name=vpc-id,Values=$vpc_id" \
        ...
        ec2_instance_id=$(aws ec2 describe-instances \
            --filters "Name=tag:Name,Values=CloudArchitect-Lab-WebServer" "Name=instance-state-name,Values=running,pending,stopped" \
            --query 'Reservations[0].Instances[0].InstanceId' \
            --output text 2>/dev/null)
        
        if [ "$ec2_instance_id" != "None" ] && [ -n "$ec2_instance_id" ]; then
            echo "✅ EC2 Instance: CloudArchitect-Lab-WebServer ($ec2_instance_id)"
        fi
    ...
    echo "• 학생이 생성한 Auto Scaling 리소스는 별도로 삭제해야 합니다"
}
...
cleanup_ec2_instances() {
    ...
    local ec2_instances=$(aws ec2 describe-instances \
        --filters "Name=tag:Name,Values=CloudArchitect-Lab-WebServer" \
    ...
    if [ ! -z "$ec2_instances" ] && [ "$ec2_instances" != "None" ]; then
        for instance_id in $ec2_instances; do
            show_info "EC2 인스턴스 종료 중: CloudArchitect-Lab-WebServer ($instance_id)"
...
cleanup_security_groups() {
    ...
    # Lab06 setup에서 생성한 Security Group 찾기
    ...
    if [ "$sg_id" != "None" ] && [ ! -z "$sg_id" ]; then
        # Security Group을 사용 중인 인스턴스 확인
        local sg_in_use=$(aws ec2 describe-instances \
            --filters "Name=instance.group-id,Values=$sg_id" "Name=instance-state-name,Values=running,stopped,pending" \
            --query 'Reservations[*].Instances[*].InstanceId' \
            --output text 2>/dev/null)
        
        if [ -z "$sg_in_use" ] || [ "$sg_in_use" = "None" ]; then
            show_info "Security Group 삭제 중: CloudArchitect-Lab-WebServer-SG ($sg_id)"
            ...
            while [ $delete_attempts -lt $max_attempts ]; do
                if aws ec2 delete-security-group --group-id "$sg_id" >/dev/null 2>&1; then
                    show_success "Security Group 삭제 완료: CloudArchitect-Lab-WebServer-SG ($sg_id)"
                    break
                else
                    ((delete_attempts++))
                    if [ $((delete_attempts % 10)) -eq 0 ]; then
                        show_info "Security Group 삭제 재시도 중... (시도 ${delete_attempts}/${max_attempts}회)"
                    fi
                    if [ $delete_attempts -ge $max_attempts ]; then
                        show_error "Security Group 삭제 실패: 최대 재시도 횟수 초과"
                        return 1
                    fi
                    sleep 3
                fi
            ...
            show_warning "Security Group이 인스턴스에서 사용 중입니다: $sg_in_use"
            show_info "학생이 생성한 Auto Scaling 리소스를 먼저 삭제한 후 이 스크립트를 다시 실행하세요."
        fi
    else
        show_info "Security Group이 존재하지 않습니다."
    fi
...
cleanup_internet_gateways() {
    ...
    if [ ! -z "$igw_ids" ] && [ "$igw_ids" != "None" ]; then
        for igw_id in $igw_ids; do
            ...
            show_success "Internet Gateway 삭제 완료: $igw_name ($igw_id)"
        done
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        for subnet_id in $subnet_ids; do
            ...
            if [ "$eni_count" -gt 0 ]; then
                show_warning "서브넷에 네트워크 인터페이스가 $eni_count 개 연결되어 있습니다. (Auto Scaling 인스턴스가 실행 중일 수 있음)"
                show_info "학생이 생성한 Auto Scaling Group을 먼저 삭제한 후 이 스크립트를 다시 실행하세요."
                sleep 5
                continue
            fi
    ...
    if [ ! -z "$vpc_ids" ] && [ "$vpc_ids" != "None" ]; then
        for vpc_id in $vpc_ids; do
            ...
            if [ ! -z "$enis" ] && [ "$enis" != "None" ]; then
                ...
                show_info "학생이 생성한 Auto Scaling Group과 Load Balancer를 먼저 삭제한 후 이 스크립트를 다시 실행하세요."
                return 1
            fi
...
show_completion_summary() {
    echo ""
    show_success "🎉 Lab06 Auto Scaling 실습 인프라 정리가 완료되었습니다!"
    echo ""
    
    echo "📋 삭제 완료된 리소스:"
    echo "✅ VPC: CloudArchitect-Lab-VPC"
    echo "✅ Internet Gateway: CloudArchitect-Lab-IGW"
    echo "✅ Public Subnets: CloudArchitect-Lab-Public-Subnet-1/2"
    echo "✅ Route Tables: 사용자 정의 라우트 테이블"
    echo "✅ Security Groups: CloudArchitect-Lab-WebServer-SG"
    echo "✅ EC2 Instance: CloudArchitect-Lab-WebServer"
    echo ""
    
    echo "💰 비용 절약"
    echo "• 모든 Lab06 관련 리소스가 정리되어 추가 비용이 발생하지 않습니다"
    ...
    echo "• VPC: CloudArchitect-Lab-VPC"
    echo "• Subnets: 모든 Public 서브넷"
    echo "• Route Tables: 사용자 정의 라우트 테이블"
    echo "• Security Groups: CloudArchitect-Lab-WebServer-SG"
    echo "• Internet Gateway: CloudArchitect-Lab-IGW"
    echo "• EC2 Instances: CloudArchitect-Lab-WebServer"
    echo ""
...
main() {
    ...
    # 헤더 표시
    echo "================================"
    echo "Lab06: Amazon EC2 Auto Scaling 구성 정리"
    echo "================================"
    echo "목적: Lab06에서 생성된 모든 AWS 리소스를 안전하게 정리"
    echo "================================"
    echo ""
