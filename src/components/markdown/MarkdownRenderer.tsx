...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
  ...
  const components = {
    ...
    blockquote: ({ children }: any) => {
            ...
            if (node?.type === 'p' || node?.props?.node?.tagName === 'p') {
              ...
              if (isFirstLine && Array.isArray(children)) {
                ...
                if (processedChildren.length > 0) {
                  result.push(
                    <div key={`text-${key++}`} className="alert-content-text">
                      {processUnconvertedBold(processedChildren)}
                    </div>,
                  );
                }
                return;
                ...
                if (cleaned) {
                  result.push(
                    <div key={`text-${key++}`} className="alert-content-text">
                      {processUnconvertedBold(cleaned)}
                    </div>,
                  );
                }
                return;
              ...
              if (children) {
                result.push(
                  <div key={`text-${key++}`} className="alert-content-text">
                    {processUnconvertedBold(children)}
                  </div>,
                );
              }
              return;
                ...
                if (
                  ...
                  listItems.push(
                    <li key={`li-${idx}`} className="alert-content-list-item">
                      {processUnconvertedBold(item.props?.children)}
                    </li>,
                  );
                }
              });
      ...
      // [!ARCHITECTURE] 감지 - 아키텍처 다이어그램 전용 블록
      if (content.includes('[!ARCHITECTURE]')) {
        const titleMatch = content.match(/\[!ARCHITECTURE\]\s+(.+?)(?:\n|$)/);
        const archTitle = titleMatch ? titleMatch[1].trim() : '아키텍처 다이어그램';

        // children에서 [!ARCHITECTURE] 제목 줄 제거 후 내용만 추출
        const removeArchMarker = (node: any): any => {
          if (typeof node === 'string') {
            return node.replace(/\[!ARCHITECTURE\]\s+.+?(?:\n|$)/, '');
          }
          if (Array.isArray(node)) {
            return node.map(removeArchMarker);
          }
          if (node?.props?.children) {
            return {
              ...
              props: {
                ...node.props,
                children: removeArchMarker(node.props.children),
              },
            };
          }
          return node;
        ...
        const archContent = removeArchMarker(children);
            ...
            <div className="architecture-box">
              <div className="architecture-box-header">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M3 3h18v18H3V3zm2 2v14h14V5H5zm2 2h4v4H7V7zm6 0h4v4h-4V7zm-6 6h4v4H7v-4zm6 0h4v4h-4v-4z" fill="#ff9900" />
                </svg>
                <span>{archTitle}</span>
              </div>
      ...
      return (
        <div className={`info-box info-box--${boxType}`} role={roleAttr}>
          ...
          <div className="info-box-content">
            <strong>{label}</strong>
            <div className="alert-content-wrapper">{processUnconvertedBold(cleanedChildren)}</div>
          </div>
        </div>
      );
    },
